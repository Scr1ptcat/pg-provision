...
hba_policy_block() {
	...
	if [[ "${RUNTIME_MODE:-system}" == "user" ]]; then
		printf '# User-mode local policy\n'
		if ((${#LOCAL_MAP_ENTRIES[@]} > 0)); then
			local entry dbrole
			printf '# User-mode pg_ident mappings apply only to roles listed here; socket access is managed outside HBA.\n'
			for entry in "${LOCAL_MAP_ENTRIES[@]}"; do
				[[ -z "$entry" ]] && continue
				dbrole="${entry#*:}"
				[[ -n "$dbrole" ]] && hba_emit_rule local all "$dbrole" "" peer "map=${LOCAL_PEER_MAP}"
			done
		fi
		if [[ -n "${UNIX_SOCKET_GROUP:-}" && "${UNIX_SOCKET_GROUP_EXPLICIT:-false}" == "true" ]]; then
			hba_emit_rule local all all "" peer
		else
			hba_emit_rule local all all "" trust
		fi
		if [[ "${SOCKET_ONLY:-false}" == "true" ]]; then
			...
			hba_emit_rule host replication all "127.0.0.1/32" reject
			hba_emit_rule host replication all "::1/128" reject
		else
			printf '# Loopback TCP requires password auth; trust is limited to the protected user socket\n'
			hba_emit_rule host all all "127.0.0.1/32" scram-sha-256
			hba_emit_rule host all all "::1/128" scram-sha-256
			hba_emit_rule host replication all "127.0.0.1/32" scram-sha-256
			hba_emit_rule host replication all "::1/128" scram-sha-256
		fi
		printf '%s\n' "$end"
		return 0
	fi
